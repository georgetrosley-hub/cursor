  ...
  const generateROI = useCallback(() => {
    ...
    roi.startStream({
      ...
      body: {
        ...
        account,
        competitors,
        context: `Use case: ${useCase}\nTeam size: ${teamSize || "Not specified"}\nAvg salary/cost per person: ${avgSalary ? `$${avgSalary}` : "Not specified"}\nHours/week on tasks Claude could assist: ${hoursPerWeek || "Not specified"}\nCurrent tool costs: ${currentToolCost ? `$${currentToolCost}/year` : "Not specified"}${additionalContext ? `\nAdditional context: ${additionalContext}` : ""}\n\nGenerate a detailed ROI model for Cursor adoption in this use case at ${account.name}. Use the inputs provided, and where inputs are missing, use reasonable industry estimates for a company of ${account.employeeCount.toLocaleString()} employees with ${account.developerPopulation.toLocaleString()} developers.`,
      },
    });
  ...
  return (
    ...
    >
      <SectionHeader
        ...
        subtitle="Build a data-driven business case for Cursor in any use case"
      />
