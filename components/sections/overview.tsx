    ...
    >
      <section className="space-y-4">
        ...
        <div className="max-w-4xl space-y-3">
          ...
          <p className="max-w-2xl text-[16px] leading-relaxed text-text-secondary sm:text-[18px]">
            {account.firstWedge}
          </p>
          <p className="max-w-3xl text-[14px] leading-relaxed text-text-muted">
            The opportunity is strongest when the team keeps the first motion narrow, proves safe deployment quickly,
            and uses that win to open a larger expansion story.
          </p>
        </div>
      </section>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <MetricCard
          ...
          subtitle="Current modeled pipeline across land + near-term upside"
        />
        <MetricCard
          label="Land motion"
          value={`$${account.estimatedLandValue.toFixed(2)}M`}
          subtitle="Tight initial wedge with measurable proof points"
        />
        <MetricCard
          label="Expansion upside"
          value={`$${account.estimatedExpansionValue.toFixed(2)}M`}
          subtitle="Follow-on departments already visible from account context"
        />
        <MetricCard
          label="Active support"
          value={`${championCount} threads`}
          subtitle={`${blockedCount} blocked workstreams need attention`}
        />
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.4fr)_380px]">
        <section className="rounded-[28px] border border-white/8 bg-white/[0.03] p-5 sm:p-6">
          <SectionHeader
            title="What changed"
            subtitle="The three signals that should shape how the team works this account right now."
          />
        ...
        <section className="rounded-[28px] border border-white/8 bg-white/[0.03] p-5 sm:p-6">
          ...
          <div className="mt-6 rounded-[22px] border border-white/8 bg-black/10 px-4 py-4">
            <div className="flex items-center gap-2">
              ...
              <p className="text-[12px] font-medium text-text-primary">Executive path</p>
            </div>
            <p className="mt-2 text-[13px] leading-relaxed text-text-secondary">
